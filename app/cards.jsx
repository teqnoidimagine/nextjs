...
function Cards({ category, from = 0, len = Math.PI * 2, radius = 5.25, onPointerOver, onPointerOut, ...props }) {
  ...
  function Card({ url, active, hovered, link, ...props }) {
    ...
    const handleClick = () => {
      window.location.href = link // Route to the specified link
    }
...
function ActiveCard({ hovered, ...props }) {
  ...
  return (
    <Billboard {...props}>
      ...
      <Image ref={ref} transparent radius={0.3} position={[0, 1.5, 0]} scale={[3.5, 1.618 * 3.5, 0.2, 1]} url={`https://picsum.photos/seed/${hovered}/400/600`} /> {/* Dummy image URL */}
    </Billboard>
  )
}
